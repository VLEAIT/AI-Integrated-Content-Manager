...
class PostMasterModel(Base,TimestampMixin):
    ...
    workspace_id:Mapped[int]=mapped_column(ForeignKey("workspace.id", ondelete="CASCADE"),nullable=True,default=None)
    creator_id:Mapped[int]=mapped_column(ForeignKey("users.id", ondelete="CASCADE"),nullable=True,default=None)

    ai_caption:Mapped[Optional[str]]=mapped_column(Text,nullable=True,default=None)
    caption_ai_status:Mapped[AIstatus]=mapped_column(SQLEnum(AIstatus),default=AIstatus.QUEUED,nullable=False)
    caption_ai_error:Mapped[Optional[str]]=mapped_column(Text,nullable=True,default=None)
 

    children:Mapped[List["PostChildModel"]]=relationship(back_populates="master", cascade="all, delete-orphan",init=False)
...
class PostChildModel(Base,TimestampMixin):
    ...
    scheduled_time:Mapped[datetime]=mapped_column(DateTime(timezone=True),nullable=True,default=None)
    published_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),nullable=True,default=None)


    master:Mapped["PostMasterModel"]=relationship(back_populates="children",init=False)
